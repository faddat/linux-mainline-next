...
static int sun8i_emac_rx_sk(struct net_device *ndev, int i)
{
	...
	/* TODO need to recheck this value */
	sk = netdev_alloc_skb_ip_align(ndev, 2000);
	if (!sk)
		return -ENOMEM;
				   ...
				   unsigned char *addr, int index)
{
	...
	dev_info(priv->dev, "device MAC address %02x:%02x:%02x:%02x:%02x:%02x\n",
...
static int sun8i_emac_complete_xmit(struct net_device *ndev)
{
	...
	for (i = 0; i < nbdesc; i++) {
		...
		if (ddesc->status != 0 || ddesc->st) {
			...
			dev_dbg(priv->dev, "%s found slot to clean %d at %pad %x %x (len=%d)\n",
				__func__, i, &ddesc, ddesc->status, ddesc->st,
				frame_len);
			if (priv->tx_sk[i]) {
				dev_kfree_skb_irq(priv->tx_sk[i]);
				priv->tx_sk[i] = NULL;
			...
			ddesc->status = 0;
			ddesc->st = 0;
			priv->ndev->stats.tx_bytes += frame_len;
		}
	}
	return 0;
}
...
static netdev_tx_t sun8i_emac_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	dev_dbg(priv->dev, "%s len=%u skblen=%u\n", __func__, len, skb->len);
	...
	if (ddesc->status == 0) {
		i = priv->tx_slot;
		rb_inc(&priv->tx_slot);
		dev_dbg(priv->dev, "%s found txslot %d at %pad (txslot=%d)\n",
		...
		dev_dbg(priv->dev, "%s found slot %d at %pad (txslot=%d)\n",
			__func__, i, &ddesc, priv->tx_slot);
	}
	...
	if (n > 0) {
		for (nf = 0; nf < n; nf++) {
			...
			/* TODO check used*/
			if (ddesc->st & BIT(31))
				dev_err(priv->dev, "Already used\n");
			ddesc->st = skb_frag_size(frag);
			ddesc->st |= BIT(24);
	...
	rb_inc(&i);
	do {
		ddesc = priv->dd_tx + rbd_i;
		ddesc->st |= BIT(27);
		ddesc->st |= BIT(28);
		/* DMA can work on it */
		ddesc->status = BIT(31);
		dev_dbg(priv->dev, "xmit final %02d %x %x\n", rbd_i,
			ddesc->status, ddesc->st);
		rb_inc(&rbd_i);
	} while (rbd_i != i);
	priv->tx_slot = i;
